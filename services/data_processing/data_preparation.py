...
from utils.data_helper.quarterly_helper import (
    find_first_quarter,
    find_last_quarter,
    create_quarterly_data,
)
from utils.data_helper.fill_nan_helper import (
    predict_current_quarter_value_daily,
    predict_current_quarter_value_monthly,
    missing_last_values,
)
import numpy as np
...
MONTHLY_METRICS = [
    ...
    (
        ...
        "NZAC component - Card transaction spend",
        True,
    ),
    (
        "New Zealand Activity Index (NZAC)",
        "NZAC component - Electricity grid demand",
        True,
    ),
    ("New Zealand Activity Index (NZAC)", "NZAC component - Manufacturing index", True),
    (
        "New Zealand Activity Index (NZAC)",
        "NZAC component - New jobs posted online",
        True,
    ),
    (
        "New Zealand Activity Index (NZAC)",
        "NZAC component - Traffic index (heavy)",
        True,
    ),
    (
        "New Zealand Activity Index (NZAC)",
        "NZAC component - Traffic index (light)",
        True,
    ),
...
def data_download():
    """
    Download data from S3 and do some data cleaning
    Return Pandas object
    """
    data_df = download_from_s3_return_df("api-data.csv", "nz-stats/stats-api/")
    data_df = data_df[
        (data_df["ResourceID"].isin(["CPTRD2", "CPTRD4", "CPTRD1", "CPTRD5"]))
        | (data_df["Duration"].notnull())
    ].copy()
    data_df["Period"] = pd.to_datetime(data_df["Period"])
    data_df.fillna("", inplace=True)
    data_df.loc[
        (data_df["Duration"].isnull()) | (data_df["Duration"] == ""), "Duration"
    ] = "P1D"
...
def create_quarterly_data_from_daily_metrics(df):
    ...
    for title, label1, is_true in DAILY_METRICS:
        for label2 in df.loc[
            (df["Title"] == title) & (df["Label1"] == label1), "Label2"
        ].unique():
            for label3 in df.loc[
                (df["Title"] == title)
                & (df["Label1"] == label1)
                & (df["Label2"] == label2),
                "Label3",
                ...
                quarterly = predict_current_quarter_value_daily(
                    quarterly, curr_qt, is_true
                )
    ...
    for title, label1, is_true in MONTHLY_METRICS:
        for label2 in df.loc[
            ...
            for label3 in df.loc[
                ...
                temp.set_index("Period", inplace=True)
                quarterly, curr_qt = create_quarterly_data(temp, col_name, is_true)
                quarterly = predict_current_quarter_value_monthly(
                    quarterly, curr_qt, is_true
                )
...
def data_preparation():
    ...
    ## upload
    upload_str_to_s3(
        prepared_df.to_csv(), "machine_learning/preprocessing/quarterly.csv"
    )
