...
def prediction(df, selected_models):
    ...
    for i in range(int(len(df) * ALPHA), len(df)):
        ...
        fit = SimpleExpSmoothing(x, initialization_method="heuristic").fit(
    ...
    for name, model in models_cv:
        ...
        for i in np.arange(size, len(df) + (nan_length + 1)):
            ...
            model.fit(X, y)
            y_pred.append(
                model.predict(df.iloc[[i]].drop(["national_gdp"], axis=1).values)[0]
            )
            if nan_length < -1:
                if i == len(df) + nan_length:
                    ...
                    y_pred.append(
                        model.predict(
                            df.iloc[[i + 1]].drop(["national_gdp"], axis=1).values
                        )[0]
                    )
            if (
                name == "XGBRegressor"
            ):  # xgboost model usually outperforms other models, But it needs to be dynamic
                xgb_model = model
        result[name] = y_pred
    ...
    upload_str_to_s3(
        result.to_csv(), f"machine_learning/results/national_gdp_{month_str}.pkl"
    )
